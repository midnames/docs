...
The SDK uses a `Result` pattern for type-safe error handling. All async functions return a `Result` type.
...
- Check the [error handling guide](/reference/errors/) for error management patterns
