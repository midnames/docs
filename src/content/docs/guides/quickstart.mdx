...
That's it! The SDK works out of the box with sensible defaults.
...
All SDK functions return a `Result` type for type-safe error handling:
...
By default, the SDK connects to Midnight TestNet. You can customize this:
...
Target: ${resolvedTarget || 'Not set'}
Name: ${fields.get('name') || 'N/A'}
Bio: ${fields.get('bio') || 'N/A'}
Website: ${fields.get('website') || 'N/A'}
  `);
}
...
```bash
# Run it
npx ts-node app.ts alice.night
```
