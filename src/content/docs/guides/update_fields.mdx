...
3. **SDK setup** with wallet providers (see [Domain Operations](/reference/operations/#prerequisites))
...
For efficiency, update multiple fields in a single transaction:
...
<Aside type="tip">
  **Maximum 10 fields per transaction.** If you need to set more, split them into multiple `addMultipleFields()` calls.
</Aside>
...
```typescript
// ✅ Good - recognized by all MNS apps
await insertField('alice.night', 'twitter', '@alice', providers);

// ⚠️ Less ideal - custom field name
...
```typescript
// ❌ Less efficient - 3 separate transactions
...
```typescript
async function updateFieldWithFeedback(domain: string, key: string, value: string, providers) {
...
// IPFS Gateway
await insertField('alice.night', 'avatar', 'https://ipfs.io/ipfs/Qm123abc...', providers);

// Midnight NFT Contract (contractAddress:tokenId)
...
- Check wallet balance (need funds for gas)
...
- Learn about [domain operations](/reference/operations/) for more write operations
